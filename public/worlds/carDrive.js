...
var carDrive = function() {
    this.rearWheelJoint = null;
    this.moveFlags = 0;
    this.carBody = null;    
}

carDrive.prototype.setNiceViewCenter = function() {
    PTM = 28;
    setViewCenterWorld(new b2Vec2(0,1), true);
}

carDrive.prototype.setup = function() {
    ...
    var ground = world.CreateBody(new b2BodyDef());
    {
        ...
        shape.Set(new b2Vec2(-20.0, 0.0),
            new b2Vec2(20.0, 0.0)
        );
        ground.CreateFixture(fd);

        var hs = [0.25, 1.0, 4.0, 0.0, 0.0,
            -1.0, -2.0, -2.0, -1.25, 0.0
        ];
    ...
    /* Car */
    {
        ...
        jd.Initialize(this.carBody,
            wheelBody1,
            wheelBody1.GetPosition(),
            axis
        );
        ...
        this.rearWheelJoint = Box2D.castObject(
            world.CreateJoint(jd),
            b2WheelJoint
        );
        ...
        jd.Initialize(this.carBody,
            wheelBody2,
            wheelBody2.GetPosition(),
            axis
        );
        jd.set_enableMotor(false);
        jd.set_frequencyHz(m_hz);
        jd.set_dampingRatio(m_zeta);
        wheelJoint2 = Box2D.castObject(
            world.CreateJoint(jd),
            b2WheelJoint
        );
    }
...
carDrive.prototype.updateMotorSpeed = function() {
...
carDrive.prototype.step = function() {
    this.updateMotorSpeed();

    //move camera to follow car
    var pos = this.carBody.GetPosition();
    var vel = this.carBody.GetLinearVelocity();
    var futurePos = new b2Vec2(
        pos.get_x() + 0.15 * vel.get_x(),
        pos.get_y() + 0.15 * vel.get_y()
    );
    setViewCenterWorld( futurePos );
}

carDrive.prototype.onKeyDown = function(canvas, evt) {
    if ( evt.keyCode == 74 ) {//j
...
carDrive.prototype.onKeyUp = function(canvas, evt) {    
    if ( evt.keyCode == 74 ) {//j
        this.moveFlags &= ~MOVE_LEFT;
        this.updateMotorSpeed();
    }
